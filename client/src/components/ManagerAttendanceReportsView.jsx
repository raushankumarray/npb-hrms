...
export default function ManagerAttendanceReportsView({ user, company = {}, onStatsUpdate }) {
  ...
  // FILTER 1: Date Filter Mode ('today' | 'month' | 'custom')
  const [dateFilterMode, setDateFilterMode] = useState('today'); // 'today', 'month', 'custom'
  ...
  const handleExport = async (format) => {
    ...
    try {
      ...
      if (res.isBlob) {
        ...
        a.download = `Daily_Attendance_Report_${timestamp}.${format === 'pdf' ? 'pdf' : 'xlsx'}`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
        setSuccess(`Exported ${format.toUpperCase()} attendance report successfully.`);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <button
              ...
              disabled={exporting}
              className="px-3 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold flex items-center gap-1.5 shadow-sm transition-colors disabled:opacity-50"
            ...
            <button
              ...
              disabled={exporting}
              className="px-3 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold flex items-center gap-1.5 shadow-sm transition-colors disabled:opacity-50"
              title="Download filtered report in PDF"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          ...
          <span className="text-[11px] text-slate-400">
            Active view: <strong className="text-slate-700">{dateFilterMode === 'today' ? 'Today' : dateFilterMode === 'month' ? `${MONTH_NAMES[filterMonth - 1]} ${filterYear}` : `${customFromDate} to ${customToDate}`}</strong>
          </span>
        </div>
