...
export default function ManagerAttendanceReportsView({ user, company = {}, onStatsUpdate }) {
  ...
  const handleExport = async (format, scopeOverride = null) => {
    ...
    try {
      ...
      // If targetScope === 'all', no date params -> exports all recorded dates
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {/* Action Buttons: Excel Upload, Manual Entry, Excel Export, PDF Export */}
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {/* Record Attendance Manually */}
            <button
              type="button"
              onClick={openNewManualModal}
              className="px-3 py-2 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-xl font-bold flex items-center gap-1.5 shadow-xs transition-colors"
              title="Manually record or adjust attendance for a team member"
            >
            ...
            <div className="relative group">
              <button
                ...
                disabled={exporting}
                className="px-3 py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-xl font-bold flex items-center gap-1.5 shadow-sm transition-colors disabled:opacity-50"
                title="Download filtered report in Excel (.xlsx)"
              >
                <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-300" />
                <span>Export Excel</span>
                <ChevronDown className="w-3 h-3 text-emerald-200" />
              </button>
              <div className="hidden group-hover:block absolute right-0 top-full mt-1 w-56 bg-white border border-slate-200 rounded-xl shadow-xl z-30 py-1 text-slate-700 text-xs">
                <button
                  type="button"
                  onClick={() => handleExport('xlsx')}
                  className="w-full text-left px-3 py-2 hover:bg-slate-50 font-semibold text-slate-800 flex items-center justify-between"
                >
                  <span>Current Filtered View</span>
                  <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-mono">.xlsx</span>
                </button>
                <button
                  type="button"
                  onClick={() => handleExport('xlsx', 'today')}
                  className="w-full text-left px-3 py-2 hover:bg-emerald-50 text-slate-700 flex items-center justify-between"
                >
                  <span>Today (All Staff Real-Time)</span>
                  <span className="text-[10px] bg-emerald-100 text-emerald-700 font-bold px-1.5 py-0.5 rounded">Today</span>
                </button>
                <button
                  type="button"
                  onClick={() => handleExport('xlsx', 'month')}
                  className="w-full text-left px-3 py-2 hover:bg-sky-50 text-slate-700 flex items-center justify-between"
                >
                  <span>Full Month ({MONTH_NAMES[filterMonth - 1]})</span>
                  <span className="text-[10px] bg-sky-100 text-sky-700 font-bold px-1.5 py-0.5 rounded">Month</span>
                </button>
                <button
                  type="button"
                  onClick={() => handleExport('xlsx', 'all')}
                  className="w-full text-left px-3 py-2 hover:bg-purple-50 text-slate-700 flex items-center justify-between"
                >
                  <span>All Recorded Dates History</span>
                  <span className="text-[10px] bg-purple-100 text-purple-700 font-bold px-1.5 py-0.5 rounded">All</span>
                </button>
              </div>
            </div>
            ...
            <div className="relative group">
              <button
                ...
                disabled={exporting}
                className="px-3 py-2 bg-rose-700 hover:bg-rose-800 text-white rounded-xl font-bold flex items-center gap-1.5 shadow-sm transition-colors disabled:opacity-50"
                title="Download filtered report in PDF / Printable"
              >
              ...
              <div className="hidden group-hover:block absolute right-0 top-full mt-1 w-56 bg-white border border-slate-200 rounded-xl shadow-xl z-30 py-1 text-slate-700 text-xs">
                ...
                <button
                  type="button"
                  onClick={() => handleExport('pdf', 'today')}
                  className="w-full text-left px-3 py-2 hover:bg-emerald-50 text-slate-700 flex items-center justify-between"
                >
                  <span>Today (All Staff Real-Time)</span>
                  <span className="text-[10px] bg-emerald-100 text-emerald-700 font-bold px-1.5 py-0.5 rounded">Today</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          ...
          <span className="text-[11px] text-slate-400">
            Active view: <strong className="text-slate-700">{dateFilterMode === 'today' ? `Today (${todayStr}) - All Team Status` : dateFilterMode === 'month' ? `${MONTH_NAMES[filterMonth - 1]} ${filterYear}` : dateFilterMode === 'custom' ? `${customFromDate} to ${customToDate}` : 'All Recorded Dates'}</strong>
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          {/* COLUMN 1: FILTER DATE DROPDOWN (Today, Month, Custom, All) */}
          <div>
            ...
            <div className="relative">
              ...
              >
                <option value="today">Today ({todayStr}) - All Team Status</option>
                <option value="month">Month-wise (Full Month)</option>
                <option value="custom">Custom Date Range</option>
                <option value="all">All Dates (All Recorded Data)</option>
              </select>
            </div>
            ...
            {dateFilterMode === 'all' && (
              <div className="mt-2 p-2 bg-purple-50 border border-purple-200/80 rounded-lg text-[11px] text-purple-800 flex items-center gap-1.5 font-medium">
                <Calendar className="w-3.5 h-3.5 text-purple-600 shrink-0" />
                <span>Showing attendance records across all dates.</span>
              </div>
            )}
        ...
        {/* Quick Date Scope & One-Click Download Row */}
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-100 text-xs">
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mr-1">Quick Select:</span>
            <button
              type="button"
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-medium text-slate-400">Quick Download:</span>
            <button
              type="button"
              onClick={() => handleExport('xlsx', 'today')}
              disabled={exporting}
              ...
              title="Download Today's Attendance for all team members in Excel"
            >
              <Download className="w-3 h-3 text-emerald-600" />
              <span>Today (.xlsx)</span>
            </button>
            <button
              type="button"
              onClick={() => handleExport('xlsx', 'month')}
              disabled={exporting}
              className="px-2.5 py-1 bg-sky-50 hover:bg-sky-100 text-sky-800 border border-sky-200 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors"
              title="Download Full Month Attendance for all team members in Excel"
            >
              <Download className="w-3 h-3 text-sky-600" />
              <span>Month (.xlsx)</span>
            </button>
            <button
              type="button"
              onClick={() => handleExport('pdf', 'today')}
              disabled={exporting}
              className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-800 border border-rose-200 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors"
              title="Download Today's Attendance for all team members in PDF"
            >
